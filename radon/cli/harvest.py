                ...
                for k, v in results._asdict().items():
                    if k == "functions":
                        result[filename]["functions"] = {
                            key: val._asdict() for key, val in v
                        }
                    else:
...
class LCOMHarvester(Harvester):
    ...
    def as_json(self):
        """Format the results as JSON."""
        result_dict = dict(self.results)
        return json.dumps(result_dict)

    def to_terminal(self):
        """Yield lines to be printed to a terminal."""
        for filename, classes in self.results:
            if "error" in classes:
                yield filename, (classes["error"],), {"error": True}
                continue
            yield filename, (), {}
            for class_name, lcom_value in classes.items():
                yield "    {0}: LCOM = {1}", (class_name, lcom_value), {}


from radon.visitors import analyze_cbo
    ...
    def to_terminal(self):
        ...
        for filename, classes in self.results:
            ...
            for class_name, data in classes.items():
                cbo_value = data["cbo"]
                coupled_classes = ", ".join(data["coupled_classes"])
                yield "    {0}: CBO = {1}", (class_name, cbo_value), {}
                if coupled_classes:
                    yield "        Coupled Classes: {0}", (coupled_classes,), {}
